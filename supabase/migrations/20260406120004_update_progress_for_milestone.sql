-- Migration: Update calculate_goal_progress to support milestone goals
-- Milestone goals report 100% when completed, 0% otherwise
...
begin
  ...
  -- Milestone goals: progress is based on status (completed = 100%)
  if v_goal.goal_type = 'milestone' then
  ...
  if v_goal.target_value is not null and v_goal.target_value > 0 then
